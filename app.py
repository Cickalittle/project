...
class EnigmaRotor:
    """Simulazione di un rotore Enigma"""
    def __init__(self, name: str, start: str = 'A'):
        data = ENIGMA_ROTORS[name]
        self.wiring = data['wiring']
        self.notch  = data['notch']
        self.pos    = ord(start.upper()) - 65

    def step(self):
        self.pos = (self.pos + 1) % 26

    def at_notch(self) -> bool:
        return chr(self.pos + 65) == self.notch

    def forward(self, c: int) -> int:
        shifted = (c + self.pos) % 26
        out = ord(self.wiring[shifted]) - 65
        return (out - self.pos + 26) % 26

    def backward(self, c: int) -> int:
        shifted = (c + self.pos) % 26
        idx = self.wiring.index(chr(shifted + 65))
        return (idx - self.pos + 26) % 26

    def get_position(self) -> str:
        return chr(self.pos + 65)


def enigma_encrypt(text: str, r1: str, r2: str, r3: str, key: str) -> dict:
    key = (key + 'AAA')[:3].upper()
    rotor1 = EnigmaRotor(r1, key[0])
    rotor2 = EnigmaRotor(r2, key[1])
    rotor3 = EnigmaRotor(r3, key[2])
    ...
    for ch in text.upper():
        ...
        # Double-stepping
        r2_notch = rotor2.at_notch()
        r3_notch = rotor3.at_notch()
        if r2_notch:
            rotor2.step()
            rotor1.step()
        elif r3_notch:
        ...
        n = rotor2.forward(n)
        n = rotor1.forward(n)
        n = ord(ENIGMA_REFLECTOR[n]) - 65
        n = rotor1.backward(n)
        n = rotor2.backward(n)
        n = rotor3.backward(n)
        result.append(chr(n + 65))

    return {
        'output': ''.join(result),
        'final_positions': {
            'r1': rotor1.get_position(),
            'r2': rotor2.get_position(),
            'r3': rotor3.get_position(),
        }
    }
...
def api_enigma():
    ...
    try:
        text = data.get('text', '')
        r1   = data.get('rotor1', 'I')
        r2   = data.get('rotor2', 'II')
        r3   = data.get('rotor3', 'III')
        key  = data.get('key', 'AAA')

        if r1 not in ENIGMA_ROTORS or r2 not in ENIGMA_ROTORS or r3 not in ENIGMA_ROTORS:
            return jsonify({'error': 'Rotore non valido'}), 400

        result = enigma_encrypt(text, r1, r2, r3, key)
        return jsonify(result)
